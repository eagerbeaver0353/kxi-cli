...
def test_options_hostname_prompt(capsys, mocker, monkeypatch):
    ...
    assert "Must either set command line arg --hostname or configuration value hostname" in e.value.message
...
def test_options_filepath_prompt():
    ...
    assert isinstance(e.value, click.ClickException)
    assert 'Must either set command line arg --filepath or configuration value install.filepath' in e.value.message
