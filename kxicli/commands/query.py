...
def query(hostname, usage, client_id, client_secret, sql, output_format, output_file):
    """Execute a SQL query and print results to console/file"""
    hostname = options.hostname.prompt(hostname, silent=True)
    conn = Query(hostname, usage=usage.upper(), client_id=client_id, client_secret=client_secret)
    df = conn.sql(sql).pd()
